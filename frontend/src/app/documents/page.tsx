...
function DocumentVaultContent() {
  ...
  return (
    <div className="relative min-h-screen bg-(--bg-base) px-4 sm:px-6 lg:px-8 py-6 sm:py-8 lg:py-10 overflow-hidden">
      ...
      <div className="relative z-10">
        ...
        <div className="mb-4 sm:mb-6 lg:mb-8">
          ...
          <p className="text-(--text-secondary) text-xs sm:text-sm lg:text-base leading-relaxed max-w-full sm:max-w-2xl">
        ...
        {loading ? (
          ...
          <>
            {/* Team status banner */}
            <div className="mt-4 sm:mt-6 lg:mt-8 mb-6 sm:mb-8 lg:mb-10 p-4 sm:p-6 lg:p-8 bg-(--bg-surface) border border-(--accent-cyan) rounded-lg flex flex-col sm:flex-row sm:items-center gap-4 sm:gap-6 lg:gap-8 shadow-(--glow-cyan-sm)">
              <div className="flex items-center gap-3 sm:gap-4 flex-1">
                ...
                <div>
                  <div className="font-display text-sm sm:text-lg lg:text-xl text-(--accent-green) tracking-widest">FINALIST ONSITE</div>
                  {docInfo && <div className="text-[8px] sm:text-xs lg:text-sm text-(--text-muted)">Verification Code: GX-{docInfo.id.slice(0, 8).toUpperCase()}</div>}
                </div>
              </div>
            ...
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 sm:gap-4 mb-4 sm:mb-6 lg:mb-8">
              ...
              <div className="font-mono text-[8px] sm:text-xs lg:text-sm text-(--text-muted) whitespace-nowrap">FORMAT: PDF/A-3 COMPLIANT</div>
            </div>
              ...
              {(team?.members ?? []).map((member, i) => (
                <div key={i} className="flex flex-col sm:flex-row items-start sm:items-center gap-3 sm:gap-4 p-3 sm:p-4 lg:p-5 bg-(--bg-surface) border border-(--border-subtle) rounded-lg hover:border-(--border-active) transition-colors">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="text-[8px] sm:text-xs lg:text-sm text-(--text-secondary) truncate">{member.email}</div>
                    {member.isLeader && <div className="text-[8px] sm:text-xs lg:text-sm text-(--accent-green) mt-1">TEAM LEADER</div>}
                  </div>
                  <div className="flex items-center gap-2 sm:gap-3 flex-shrink-0">
                    <button className="p-1.5 sm:p-2 hover:bg-(--bg-elevated) rounded transition-colors" title="View">
                      <Eye size={16} className="text-(--accent-cyan)" />
                    </button>
                    <button className="p-1.5 sm:p-2 hover:bg-(--bg-elevated) rounded transition-colors" title="Download">
