...
echo "📦 Creating compressed archive (INCLUDING .env.production)..."
tar -czf deploy.tar.gz \
  --exclude='node_modules' \
  --exclude='.git' \
  --exclude='.next' \
  --exclude='.env' \
  ./backend ./frontend ./docker-compose.yml ./docker-compose.prod.yml ./package.json ./.env.production
...
echo "🔧 Extracting and restarting containers (using .env.production)..."
...
echo "✅ SUCCESS: Project deployed with .env.production on the server!"
